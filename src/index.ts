...
app.use(morgan('dev', { stream: { write: s => logger.debug(s.trimEnd()) }, skip: req => /^\/(js|css)/.test(req.url) }));
app.use(cors({ origin: '*' }));
...
if (require.main === module) {
  ...
  server.listen(port ?? 0, () => logger.info(chalk`Listening on port {yellow ${(server.address() as AddressInfo).port}}`));
} else {
